...
class LogHelper 
{
    ...
    {
        // 添加调试输出，确认方法被调用
        echo "LogHelper::debug 被调用: " . $message . PHP_EOL;
        
        if (env('APP_DEBUG') || env('DEBUG_LOG')) {
            echo "条件满足，开始写入日志" . PHP_EOL;
            Log::channel('debug')->debug($message . (!empty($data) ? ' 数据：' . json_encode($data, JSON_UNESCAPED_UNICODE) : ''));
        } else {
            echo "条件不满足: APP_DEBUG=" . var_export(env('APP_DEBUG'), true) . ", DEBUG_LOG=" . var_export(env('DEBUG_LOG'), true) . PHP_EOL;
        }
    }
    ...
    {
        echo "LogHelper::business 被调用: " . $message . PHP_EOL;
    ...
    {
        echo "LogHelper::error 被调用: " . $message . PHP_EOL;
        $errorMsg = $message;
    ...
    {
        echo "LogHelper::warning 被调用: " . $message . PHP_EOL;
    ...
    {
        echo "LogHelper::info 被调用: " . $message . PHP_EOL;
